...
export default function ThreeUpCarousel({ slides, interval = 3000 }: CarouselProps) {
  ...
  const isCenter = (index: number) => {
    ...
    return ((index - current + total) % total) === 0;
  };
  ...
  const getSlideStyles = (index: number) => {
    ...
    // Right slide
    if (offset === 1) return "z-10 scale-90 opacity-85 translate-x-[60%]";

    // Left slide
    if (offset === total - 1) return "z-10 scale-90 opacity-85 -translate-x-[60%]";
